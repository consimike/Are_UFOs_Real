...
if (value !== undefined && Svalue !== undefined && Cvalue !== undefined && SHvalue !== undefined && Countrys !== undefined ) {
  
    var filtered = tableData.filter(item => {

    return (item.datetime === value && item.country === Countrys && item.state ===Svalue && item.city === Cvalue && item.shape ===SHvalue);
...
else if (SHvalue === undefined) {

  var SHfilter = tableData.filter(item => {

    return(item.shape === SHvalue);
